...
extension UIImage {
    ...
    public func coarseSegmentation() -> CGImage? {
        ...
        let testml = fcncoreml.init()
        let pixBuf = self.pixelBuffer(width: 500, height: 500)
      
        guard let output = try? testml.prediction(_0: pixBuf!)
        else {
         return nil
        }
        var index: [NSNumber] = [8, 35, 35]
        
        let outputmlarry=output._88
        let value = output._88[index]
       
        let shape = output._88.shape
        ...
        for i in 0..<w {
            for j in 0..<h {
                ...
                for k in 0..<d {
                    ...
                    itemArr.append(Int(truncating: output._88[padding + pageOffset]))
